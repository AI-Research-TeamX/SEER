...
# 问题分类器的prompt
CLASSIFIER_INSTRUCTION = """You're an expert at question classification. Given a question that is sampled from 8 distinct datasets, you can extract the keywords from the question and figure out the most possible dataset that the question belongs to. You should think step by step and output the final classification result [DATASET].
...
CLASSIFIER_INSTRUCTION_NO_INFO = """You're an expert at question classification. Given a question that is sampled from 8 distinct datasets, you can extract the keywords from the question and figure out the most possible dataset that the question belongs to. You should think step by step and output the final classification result [DATASET].
Note that your answer must include a classification result in the form of [DATASET], where DATASET is your classification result.
Please use the following format when answering:
Question: This is the question.
Answer: These are your thoughts. [DATASET]
...
CLASSIFIER_INSTRUCTION_ZERO_SHOT = """You're an expert at question classification. Given a question that is sampled from 8 distinct datasets, you can extract the keywords from the question and figure out the most possible dataset that the question belongs to. You should think step by step and output the final classification result [DATASET].
Here are descriptions of all 8 datasets:
{databases_info}(END OF DESCRIPTIONS)
Note that your answer must include a classification result in the form of [DATASET], where DATASET is your classification result.
Please use the following format when answering:
...
# 任务分解器prompt
DECOMPOSER_INSTRUCTION = """You are an expert in task decomposition. Given a complex question and a set of tools, your mission is to generate the next tool to be called and the corresponding subtask based on historical information. Your output should be in the form of [TOOL]<SUBTASK>, where [TOOL] is the next tool to complete the subtask <SUBTASK>.
...
Subtask 1: Your thoughts. [TOOL]<SUBTASK>
Result 1: The execution result of Subtask 1.
...
Subtask n: Your thoughts. [Finish]<SUBTASK>
...
# 任务求解器prompt
SOLVER_INSTRUCTION = """You are an expert in task solver. Given a complex question and a set of tools, your mission is to generate the next tool to be called and the corresponding parameters based on historical information. Your output should be in the form of <CALL_BEGIN>TOOL[PARAMETERS]<CALL_END>, where TOOL is the next tool, along with [PARAMETERS].
Here are descriptions of all available tools:
{tools_info}(END OF DESCRIPTIONS)
Please use the following format when answering(n is a natural number):
Question: The question.
Hint: The hint to answer the question.
Step 1: Your thoughts. <CALL_BEGIN>TOOL[PARAMETERS]<CALL_END>
Result 1: The execution result of Step 1.
...
Step n: Your thoughts. <CALL_BEGIN>Finish[ANSWER]<CALL_END>
Result n: The execution result of Step n.
Final Answer: The final answer.
Here are some examples:
{examples_solver}(END OF EXAMPLES)
Question: {question}
{scratchpad}"""

solver_prompt = PromptTemplate(
    input_variables=["tools_info", "examples_solver", "question", "scratchpad"],
    template = SOLVER_INSTRUCTION,
)

# 子任务处理器prompt：tool-level prompt
from prompts_tools import TOOLS_PROMPT_LIST_MS
...
HANDLER_INSTRUCTION_DIRECT = """Solve a question-answering task by using tools. Task Decomposer and Subtask Handler must work together to complete the task. Now, you are a Subtask Handler, your mission is to generate parameters of [TOOL] for completing the <SUBTASK> decomposed by the decomposer and your output should be in the form of [TOOL]<PARAMETERS>. The <PARAMETERS> denotes the needed parameters for [TOOL] to complete the <SUBTASK> correctly.
...
handler_prompt_direct = PromptTemplate(
    input_variables=["parameter_info", "examples_decomposer_handler", "question", "scratchpad"],
